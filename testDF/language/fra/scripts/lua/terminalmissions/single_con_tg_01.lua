--************************
--name		: SINGLE_CON_TG_01.lua
--ver		: 0.1f
--author	: Lupus
--date		: 2004/09/30
--lang		: fra
--desc		: terminal construction mission for first 5 Twilight Guardian Construction items TL10,20,30,40,50
--npc		: 202, 205
--translation	: Legba
--************************
--changelog:
...
function DIALOG()
	...
	NODE(5)
		if(node==state)then
		GETTARGETVALUE(0,2)
			if(result==4445)then -- Item 1
				GIVEITEM(4647)
				GIVEITEM(4648)
				GIVEITEM(4649)
				GIVESPECIALITEM(1507,1,4445)
				SAY("Voilà les plans et les composants nécessaires.\nBonne chance.")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4446)then --Item 2
				GIVEITEM(4647)
				GIVEITEM(4648)
				GIVEITEM(4650)
				GIVESPECIALITEM(1507,1,4446)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4447)then --Item 3
				GIVEITEM(4647)
				GIVEITEM(4648)
				GIVEITEM(4651)
				GIVESPECIALITEM(1507,1,4447)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4448)then --Item 4
				GIVEITEM(4647)
				GIVEITEM(4648)
				GIVEITEM(4652)
				GIVESPECIALITEM(1507,1,4448)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4449)then --Item 5
				GIVEITEM(4647)
				GIVEITEM(4648)
				GIVEITEM(4653)
				GIVESPECIALITEM(1507,1,4449)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()

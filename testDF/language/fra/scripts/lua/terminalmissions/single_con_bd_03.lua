--************************
--name		: SINGLE_CON_BD_03.lua
--ver		: 0.1f
--author	: Lupus
--date		: 2004/09/30
--lang		: fra
--desc		: terminal construction mission for last 5 Black Dragon Construction items TL110,120,130,140,150
--npc		: 132, 131
--translation	: Legba
--************************
--changelog:
...
function DIALOG()
	...
	NODE(5)
		if(node==state)then
		GETTARGETVALUE(0,2)
			if(result==4395)then -- Item 1
				GIVEITEM(4579)
				GIVEITEM(4580)
				GIVEITEM(4591)
				GIVESPECIALITEM(1507,1,4395)
				SAY("Voilà les plans et les composants nécessaires.\nBonne chance.")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4396)then --Item 2
				GIVEITEM(4579)
				GIVEITEM(4580)
				GIVEITEM(4592)
				GIVESPECIALITEM(1507,1,4396)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4397)then --Item 3
				GIVEITEM(4579)
				GIVEITEM(4580)
				GIVEITEM(4593)
				GIVESPECIALITEM(1507,1,4397)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4398)then --Item 4
				GIVEITEM(4579)
				GIVEITEM(4580)
				GIVEITEM(4594)
				GIVESPECIALITEM(1507,1,4398)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()
			elseif(result==4399)then --Item 5
				GIVEITEM(4579)
				GIVEITEM(4580)
				GIVEITEM(4595)
				GIVESPECIALITEM(1507,1,4399)
				SAY("Très bien, voilà les plans et les composants.\nBonne chance!")
				ACTIVATEDIALOGTRIGGER(0)
				SETNEXTDIALOGSTATE(3)
				ENDDIALOG()

...
factionid 	= 14 ;		-- faction
missionbaseid 	= 5020 ;	-- erster eintrag in der missionbase für diese Faction
missionnpcbase 	= 5030 ;	-- erster mission npc dieser faction
...
function DIALOG()
	...
	NODE(0)
		if (node==state) then
			...
			if (result==1) then
				...
				if (result==1) then
					SAY(""..ICEBREAKER.." chargé .... Processus de décryptage initialisé ...... Processus de décryptage effectué ......")
					ANSWER("     - Poursuivre",2) ;
				else
	...
	-- mission select
	NODE(2)
		SAY("Abandon de la file d'attente ... statut : en dérangement") ;
		MissionSelector()
	NODE(9)
		SAY("Commande traitée") ;
		ENDDIALOG()
	-- mission accepts
	NODE(20)
		if (m1>0) then
			...
			if (missionpool[m1].level==easy) then
				SAY("Nouvelle table de rediction appliquée ... relai suivant  \\\\%NPC_WORLD(0)\\%NPC_NAME(0)") ;
			end
			if (missionpool[m1].level==medium) then
				SAY("Nouvelle table de rediction appliquée ... relai suivant  \\\\%NPC_WORLD(1)\\%NPC_NAME(1)") ;
			end
			if (missionpool[m1].level==hard) then
				SAY("Nouvelle table de rediction appliquée ... relai suivant  \\\\%NPC_WORLD(2)\\%NPC_NAME(2)") ;
			end
		end
		ANSWER("Données reçues ... fermeture de la connexion ... effacement du journal des connexions  ...",29)
	NODE(21)
		if (m2>0) then
	...
	-- mission decline
	NODE(23)
		SAY("Tentatives de connexion annulées ... messages entrants sur le même port ... hôte destinataire en dérangement ...") ;
	...
	-- last npc:
	NODE(50)
		SAY("... scan port 9072 ... connexion effectuée ... réception ...") ;
	...
	-- second last npc:
	NODE(100)
		SAY("... scan port 9072 ... connexion effectuée ... parcours des tables de données ...") ;
		ANSWER("Attente de réponse",101) ;
	NODE(101)
		ACTIVATEDIALOGTRIGGER(1)
		SAY("... transfert détourné ... relai suivant \\\\%NPC_WORLD(0)\\%NPC_NAME(0)") ;
		ENDDIALOG()
	-- third last npc:
	NODE(150)
		SAY("... scan port 9072 ... connexion effectuée ... parcours des tables de données ...") ;
		ANSWER("Attente de réponse",151) ;
	NODE(151)
		ACTIVATEDIALOGTRIGGER(2)
		SAY("... transfert détourné ... relai suivant \\\\%NPC_WORLD(1)\\%NPC_NAME(1)") ;
		ENDDIALOG()
end
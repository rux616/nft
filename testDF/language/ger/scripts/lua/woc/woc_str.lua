...
function SKILLNAME(skill)
	if (skill==0) then return "keinen" end
...
function DIALOG()
	...
	NODE(10)
		...
		if (WOC_SKILL~=result) and (result~=0) then
			...
			SAY("Mein Gespür sagt mir, dass Du das alte Wissen von Ceres im Bereich "..SKILLNAME(WOC_SKILL).." nicht suchst. Sprich mit einem der anderen Großen Gelehrten. Sie werden Dir auf Deiner Suche weiterhelfen können.")
			ENDDIALOG()
		else
			if (result==0) then
				...
				if (result~=-2) then
					...
					SAY("Du erfüllst nicht die Voraussetzungen, um das Wissen von Ceres zu erwerben. Du musst den Epic Run deiner Fraktion erledigen, bevor ich Dich unterrichten kann! Komme wieder, wenn Du bereit bist, diese große Ehre zu empfangen.") 
					ENDDIALOG()
				else
					-- runner hat die vorrausetzung epic
					SAY("Was ist Dein Begehr, Fremder?")
					ANSWER("Ich möchte die Weisheit von Ceres im Bereich "..SKILLNAME(WOC_SKILL).." erwerben. Können Sie mich unterrichten, ehrbarer Gelehrter?",20)
				end
			else
	...
	NODE(20)
		...
		ANSWER("Ja, Gelehrter von Ceres. Ich bin bereit diese große Ehre zu empfangen.",22) 

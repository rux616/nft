...
faction = 4 							-- CA
factionname = "Tangent Technologies"				-- CA in text

fractionsayhello = "Waffen vertrauen wir."		-- Fraktionsansprache
fractionanswer = "An Tangent glauben wir."	-- Antwort darauf
...
function DIALOG()
	...
	NODE(152) ;
		...
		SAY("Sicher. Was möchtest Du über das neue Gebäude erfahren?") ;
	...
	NODE(153)
		-- Just Info
		SAY("Sicher. Was möchtest Du über das Gebäude wissen?") ;
		if (node==state) then
			state = 172	-- get answeres from 172
		end
	NODE(154)
	...
	NODE(200)
		...
		TAKEITEM(91)
		if (result==1) then
			SAY("Danke für das Paket. Du kannst Dich ruhig ein wenig umschauen. Wenn Du dann fertig bist gehe zurück zum Information Manager - er kann Dir noch einen anderen schnell zu erledigenden Auftrag geben.") ;
